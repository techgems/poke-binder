...
NuGet is at `1.0.0` and npm at `1.0.0`. Both are prereleases, so the version
must be explicit (or `--prerelease`) — otherwise NuGet keeps you on v0:

```bash
dotnet add package TechGems.ViteDotNet --version 1.0.0
```

```bash
cd ReactApp && npm install vite-dotnet@1.0.0
```
